...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const result = await provider.generateReading({
      ...
      context: { spread_type: spreadType, selected_cards: selectedCards },
      profile
    });
    ...
    return jsonResponse({ reading_id: reading.id, spread_type: spreadType, cards: selectedCards, ...result });
  } catch (error) {
